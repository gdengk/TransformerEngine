...
void nvshmem_wait_on_stream(uint64_t* sig_addr, int wait_kind, cudaStream_t stream){
  ...
  if (wait_kind==0){
    ...
    // NVTE_CHECK_CUDA(cuStreamWriteValue64((CUstream)cur_stream, (CUdeviceptr)sig_addr, (cuuint64_t)signal_reset, CU_STREAM_WRITE_VALUE_DEFAULT));
  }
  else if(wait_kind==2){
    cuStreamWaitValue64((CUstream)cur_stream, (CUdeviceptr)sig_addr, (cuuint64_t)wait_value, CU_STREAM_WAIT_VALUE_GEQ);
    // NVTE_CHECK_CUDA(cuStreamWaitValue64((CUstream)cur_stream, (CUdeviceptr)sig_addr, (cuuint64_t)wait_value, CU_STREAM_WAIT_VALUE_GEQ));
